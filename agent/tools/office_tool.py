...
2、格式要求：
    ...
    2）尽量不要输出连续的两个回车（因为docx文档通常通过行间距控制视觉舒适度，双回车通常在markdown文本的渲染中才有效）。
</章节编制总体要求>
...
2）章节标题的文字不要漏写，
3）特别要注意的是，'title'用于"docx_write_chapter_text"时，只是用于提供章节号供获取对应章节的模板内容，绝对不能将'title'对应内容输出到正文中（因为会和"docx_write_chapter_title"输出的标题内容重复，文档中肯定不能这样）。
''',
            'required': 'True',
        },
        {
