...
def main():
    ...
    t1.send_command(task_id=task_id, command=str(Redis_LLM_Command.ASK), question='我叫土土', temperature=0.6)
    print(f'result is:')
    for chunk in t1.get_result_gen(task_id):
        print(chunk, end='', flush=True)
    print()

    t1.send_command(task_id=task_id, command=str(Redis_LLM_Command.ASK), question='我叫什么', temperature=0.6)
